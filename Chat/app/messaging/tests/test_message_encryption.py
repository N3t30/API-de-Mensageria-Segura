...
class MessageEncryptionTestCase(APITestCase):
    ...
    def setUp(self):
        self.user1 = User.objects.create_user(username="user1", password="12345678")
        self.user2 = User.objects.create_user(username="user2", password="12345678")

        self.message = Message.objects.create(
            sender=self.user1, recipient=self.user2, content="mensagem secreta"
        )
